...
async function loadJobs() {
    ...
    try {
        ...
        let jobs = data.jobs || [];
        let totalBeforeFilter = jobs.length;

        // Filter by date range (last N days)
        if (currentFilters.days && currentFilters.days > 0) {
        ...
        // Update pagination (account for client-side filtering)
        ...
        if (currentFilters.days > 0) {
            paginationInfo.textContent = `Showing ${showing} jobs from last ${currentFilters.days} days`;
        } else {
            paginationInfo.textContent = `Showing ${start}-${end} of ${data.total || 0} jobs`;
        }

        // Update pagination buttons
        btnPrev.disabled = currentPage === 0;
        btnNext.disabled = showing < pageSize;
...
function applyFilters() {
    ...
    currentFilters.days = parseInt(document.getElementById('filter-days').value) || 0;
...
async function loadApps() {
    try {
        ...
        // Filter by current days filter if set
        if (currentFilters.days > 0) {
            const cutoffDate = new Date();
            cutoffDate.setDate(cutoffDate.getDate() - currentFilters.days);
            jobs = jobs.filter(job => {
        ...
        const apps = [...new Set(jobs.map(job => job.app_id))];
