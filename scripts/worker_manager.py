...
class ExclusiveWorkerManager:
    ...
    def cleanup_worker_gpu(self, worker_name):
        """Call worker's cleanup endpoint to free GPU memory before stopping"""
        try:
            logger.info(f"Requesting GPU cleanup for {worker_name}...")
            # Call the worker's cleanup endpoint (all workers expose this on port 8000)
            response = requests.post(
                f"http://{worker_name}:8000/cleanup",
                timeout=10
            )

            if response.status_code == 200:
                logger.info(f"✓ GPU cleanup successful for {worker_name}")
                # Give it a moment for cleanup to complete
                time.sleep(2)
                return True
            else:
                logger.warning(f"⚠ GPU cleanup returned status {response.status_code}")
                return False

        except requests.exceptions.RequestException as e:
            logger.warning(f"⚠ Could not reach {worker_name} cleanup endpoint: {e}")
            # Not a critical failure - worker might already be stopped
            return False
    ...
    def stop_worker(self, worker_name):
        ...
        try:
            # First, try to cleanup GPU memory while worker is still running
            if self.is_worker_running(worker_name):
                self.cleanup_worker_gpu(worker_name)

            # Force kill the container immediately to prevent orphaned processes
            logger.info(f"Force killing {worker_name} container...")
            subprocess.run(
                ['docker', 'compose', '-p', self.project_name, '-f', self.compose_file, 'kill', worker_name],
                stderr=subprocess.DEVNULL
            )

            # Remove the container to clean up any orphaned processes
            subprocess.run(
            ...
            logger.info(f"Waiting for GPU memory to be fully released...")
            time.sleep(5)

            logger.info(f"✓ {worker_name} stopped and GPU memory freed")
            return True

        except subprocess.TimeoutExpired:
            logger.error(f"✗ {worker_name} shutdown timeout")
            return False
