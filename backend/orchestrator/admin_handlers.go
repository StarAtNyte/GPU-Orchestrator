...
func adminWorkersStatusHandler(w http.ResponseWriter, r *http.Request) {
	...
	// Get workers from in-memory map (populated from etcd) - these are ONLINE workers
	workersList := []WorkerStatusInfo{}
	onlineWorkerIDs := make(map[string]bool)

	for workerID, workerInfo := range workers {
		onlineWorkerIDs[workerID] = true
	...
	// Add all configured worker types (from app registry) as OFFLINE if not already online
	for appID, appConfig := range appRegistry {
		if appConfig.Type != "local" {
			continue // Skip modal/cloud apps
		}

		workerName := getWorkerNameForApp(appID)

		// Check if this worker is already in the list (online)
		alreadyListed := false
		for _, w := range workersList {
			if w.WorkerID == workerName || strings.HasPrefix(w.WorkerID, workerName+"-") {
				alreadyListed = true
				break
			}
		}

		// If not online, add it as OFFLINE
		if !alreadyListed {
			// Try to get historical GPU info from metrics
			var latestVRAM int
			var latestGPUName string
			err := db.QueryRow(`
				SELECT vram_total_mb, gpu_id
				...
				ORDER BY time DESC
				LIMIT 1
			`, workerName+"%").Scan(&latestVRAM, &latestGPUName)
			...
			if err == nil && latestVRAM > 0 {
				...
				// Use configured VRAM from app config if available
				if appConfig.GPUVramGB > 0 {
					vramMB := appConfig.GPUVramGB * 1024
					vramTotal = &vramMB
				}
			}
...
func adminWorkerActionHandler(w http.ResponseWriter, r *http.Request) {
	...
	switch req.Action {
	case "start":
		// "start" action uses "switch" command since worker_manager handles exclusivity
