...
sys.path.append("/app")
from handler import Gemma4ChatHandler
...
STREAM_KEY = "jobs:gemma4-chat"
GROUP_NAME = "gemma4-chat-workers"
WORKER_ID = os.getenv("WORKER_ID", "gemma4-chat-worker-1")
APP_ID = "gemma4-chat"
QUEUE = STREAM_KEY
...
handler: Gemma4ChatHandler = None
...
class WorkerHTTPHandler(BaseHTTPRequestHandler):
    def do_POST(self):
        if self.path == "/cleanup":
            try:
                ...
                self.end_headers()
                self.wfile.write(
                    json.dumps({"status": "success", "message": "Model offloaded"}).encode()
                )
            except Exception as exc:
    ...
    def do_GET(self):
        if self.path == "/status":
            ...
            self.end_headers()
            self.wfile.write(
                json.dumps({"worker_id": WORKER_ID, "model_state": state, "app_id": APP_ID}).encode()
            )
        else:
            self.send_response(404)
            self.end_headers()
...
def main():
    ...
    logger.info(f"Starting Gemma 4 26B-A4B Chat Worker | ID={WORKER_ID}")
    ...
    while not shutdown_flag.is_set():
        try:
            entries = r.xreadgroup(
                GROUP_NAME, WORKER_ID, {STREAM_KEY: ">"}, count=1, block=2000,
            )
            if entries:
